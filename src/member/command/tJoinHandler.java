...
public class tJoinHandler implements CommandHandler  {

	//필드
	private static final String FORM_VIEW ="/view/member/tjoinForm.jsp";
	...
	private String processSubmit(HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		 * //에러정보가 담기는 Map- p598 42라인 Map<String, Boolean> errors = new HashMap<String,
		 * Boolean>(); request.setAttribute("errors",errors);
		 * 
		 * //필수입력(유효성)검사- p598 45라인 joinReq.validate(errors);
		 * 
		 * //p598 47라인 if(!errors.isEmpty()) { //에러가 있으면 회원가입페이지로 이동 return
		 * request.getContextPath()+FORM_VIEW; }
		 */
		
		if(id==null || name==null || !password.equals(confirmPassword))
			return "/view/member/tjoinForm2.jsp";
